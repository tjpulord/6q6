...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
        $fp = fopen("token_token.json", "w");
        $res = fwrite($fp, json_encode(array('expires_in'=>($expired+time()), 'access_token'=>$value)));
        fclose($fp);
		return $res;
	}
	...
	protected function getCache($cachename){
		$data = json_decode(file_get_contents("token_token.json"));
    ...
    public function uploadForeverMedia($data, $type,$is_video=false,$video_info=array()){
        ...
        // echo self::API_URL_PREFIX.self::MEDIA_FOREVER_UPLOAD_URL.'access_token='.$this->access_token.'&type='.$type;
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function getTMIndustry(){
        ...
        $result = $this->http_get(self::API_URL_PREFIX.'/template/get_industry?access_token='.$this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getTMList()
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        echo "access_token:".$this->access_token;
        $result = $this->http_get(self::API_URL_PREFIX.'/template/get_all_private_template?access_token='.$this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
