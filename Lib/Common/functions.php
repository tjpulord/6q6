 ...
 * 根据身份证号码计划周岁
 */
function getage($cardnum, $curtime=""){
    if (!$cardnum) {
        return 0;
    }
    ...
    $year  = intval(substr($cardnum, 6, 4));
    $month = intval(substr($cardnum, 10, 2));
    $day   = intval(substr($cardnum, 12, 2));

    $c_year  = intval(date("Y", $curtime));
    $c_month = intval(date("m", $curtime));
    $c_day   = intval(date("d", $curtime));

    if ($c_year == $year) {
        return 0;
    }
    //是否过了生日
    if($c_month < $month){ //未过生日
...
// 获取至今天的天数
function diffday($ts='', $timestamp=true)
{
    if (!$ts) {
        return 0;
    }
    ...
    $curdate = date_create($nowtime);
    $comdate = date_create($ts);

    $intval = date_diff($comdate, $curdate);

    return $intval->format('%R%a')-1;
}
