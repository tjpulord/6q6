...
class Chart{
  ...
  */
   ...
   * 自定义多线线图
   */
  function createmutline($title,$datas=array(),$size=40,$height=100,$width=80,$legend=""){
	require (dirname(__FILE__)."/Jpgraph/jpgraph.php");
	require (dirname(__FILE__)."/Jpgraph/jpgraph_line.php");
	...
	// $legend = json_decode($legend);
	// foreach ($legend as $k => $v) {
	// 	$legend[$k] = iconv('utf-8', 'gb2312', $v['name']);
	// }
	...
	$colors = array('blue','gold','green',"orchid","black","orangered","skyblue","yellowgreen",'firebrick',"steelblue", "violet", "brown");
	// var_dump($legend);
	// var_dump($datas);
	foreach ($legend as $it => $tv) {
		$pots = array();
		foreach ($datas as $key => $value) {
			array_push($pots, $value[$tv['id']]);
		}
		// echo "$it:";
		// var_dump($pots);

		$p1 = new LinePlot($pots);
		$p1->mark->SetWidth(4);
		// $p1->mark->SetType(MARK_FILLEDCIRCLE);
		// $p1->mark->SetFillColor("#0080C0");
		$p1->SetColor($colors[$it]);

		$p1->SetLegend(iconv('utf-8', 'gb2312', $tv['name']));
		// $p1->SetCenter();
		$graph->Add($p1);
	}
	// die();
