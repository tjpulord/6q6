...
/**
 * @cc 自动生成MessageLogController控制类
 */
class MessageLogController extends BaseController {
    ...
    function __construct()
    {
        ...
        $this->iauto = new IAutomaticController("29");
    }
    ...
    function batchadd()
    {
        $usercategorydb = M('my_category');
        $usergroup = $usercategorydb->where('category=1')->select();
        $this->assign('usergroup',$usergroup);

        if(array_key_exists("doSubmit", $this->postdata) && $this->postdata["doSubmit"])
        {
            $userdb = M('weixin_user');
            $gid = array_key_exists("gid",$this->g_params) ? $this->g_params["gid"] : "0";
            if ($gid) {
                $where = "cateid=$gid";
            }else{
                $where = "1";
            }
            $userlist = $userdb->where($where." and status=1")->select();
            $messagelist = array();
            $itemdata   = $this->postdata["info"];
            $itemdata['addtime'] = time();
            foreach ($userlist as $key => $value) {
                $itemdata['userid'] = $value['id'];
                $messagelist[] = $itemdata;
            }

            $res = M('message')->addAll($messagelist);
            $this->assign("save",$res);
        }
        $this->display();
    }
}